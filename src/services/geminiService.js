...
- ECU 전류(REC_I, CURRENT, ECU_I 등 헤더 동적 식별): 수백~수천A 단위. 운전 구간 평균 → tro_data.ecu_current_avg
- 유량(FMU, FLOW, FMU1 등 헤더 동적 식별): 운전 구간 평균 → tro_data.fmu_flow_avg
- ANU 상태 컬럼(ANU, ANU_STATUS 등): 운전 마지막 기록 값 → tro_data.anu_status ('Ready'/'Operating'/'Fault')
- ⚠️ ECU 전류가 있고 유량이 있는데 TRO가 0.0 지속이면 센서/전극 이상 가능성이 높으므로 반드시 추출
...
Step 3. ECS Data Log 섹션에서 TRO + ECU/FMU 평균 계산
  - TRO 컬럼 헤더 동적 식별 (T1, T2, TRO_B1 등), 0~15 사이 값(ppm)만 사용
  - 각 운전의 시작~종료 시간 기준, 앞뒤 10분 제외 안정 구간 평균
  - 안정 구간 데이터 없거나 운전 20분 이하 → null
  - ECU 전류(REC_I, CURRENT 등) 운전 구간 평균 → ecu_current_avg
  - 유량(FMU, FLOW 등) 운전 구간 평균 → fmu_flow_avg
  - ANU 상태 마지막 값 → anu_status ('Ready'/'Operating'/'Fault')
