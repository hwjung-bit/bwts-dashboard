...
${splitInfo ? `⚠️ [분할 PDF 안내] 이 PDF는 원본 ${splitInfo.totalPages}페이지 TOTAL LOG에서 핵심 섹션만 추출한 분할본입니다.
구성: 원본 앞 ${LARGE_PDF_START_PAGES}페이지(기본정보) + 원본 마지막 ${splitInfo.totalPages - (splitInfo.totalPages - LARGE_PDF_END_PAGES)}페이지(ECS Operation Time + ECS Data Log + 최근 Event Log).
중간 Event Log 일부는 포함되지 않았습니다. ECS Operation Time과 ECS Data Log는 이 PDF 뒷부분에 있습니다.` : `⚠️ TOTAL LOG 처리 지침 (PDF 앞부분에 Operation Event Log가 수백~수천 페이지 있는 경우)
PDF를 처음부터 순서대로 읽지 마세요. 먼저 문서 전체를 빠르게 훑어 각 섹션의 시작 위치를 파악한 뒤,
아래 우선순위 순서(Step 2 → Step 3 → Step 4)로 추출하세요.
Operation Event Log가 PDF 앞에 있더라도 반드시 마지막(Step 4)에 처리합니다.`}

Step 1. 기본 정보
- 선명, IMO 번호, 제조사, 분析 기간 추출

Step 2. ECS Operation Time (최우선 — 운전 기준표)
- 섹션명: "ECS Operation Time", "Operation Time Log", "OperationTimeReport" 등 — PDF 뒷부분에서 찾을 것
...
- 섹션명: "ECS DATA LOG", "Data Log", "DataReport", "BWTS Data" 등 — PDF 뒷부분에서 찾을 것
...
- ⚠️ 이 섹션이 수백~수천 페이지에 달하더라도 전체를 읽으려 하지 마세요.
...
function validateOperationDates(data) {
  ...
  if (invalid.length > 0) {
    const note = `운전 기록에 유효하지 않은 날짜(${invalid.join(", ")}) 감지 — PDF 날짜 오인식 가능성. 원본 확인 필요.`;
    const noteEn = `Invalid date(s) detected in operation records (${invalid.join(", ")}) — possible PDF misread. Please verify source.`;
    if (data.ai_remarks && !data.ai_remarks.includes("유효하지 않은 날짜"))
      data.ai_remarks += " " + note;
    if (data.ai_remarks_en && !data.ai_remarks_en.includes("Invalid date"))
      data.ai_remarks_en += " " + noteEn;
  }
}
...
const LARGE_PDF_START_PAGES = 5;    // 앞에서 가져올 페이지 수 (기본정보)
const LARGE_PDF_END_PAGES   = 280;  // 뒤에서 가져올 페이지 수 (Op Time + Data + 최근 Event)

async function splitLargePdf(blob) {
  ...
  const startIdx = Array.from({ length: LARGE_PDF_START_PAGES }, (_, i) => i);
  const endStart = Math.max(LARGE_PDF_START_PAGES, total - LARGE_PDF_END_PAGES);
  const endIdx   = Array.from({ length: total - endStart }, (_, i) => endStart + i);

  const newDoc = await PDFDocument.create();
  const copied = await newDoc.copyPagesFrom(srcDoc, [...startIdx, ...endIdx]);
  copied.forEach((p) => newDoc.addPage(p));
  const bytes = await newDoc.save();

  const extracted = startIdx.length + endIdx.length;
  console.log(`[PDF Split] ${total}p → 앞 ${startIdx.length}p + 뒤 ${endIdx.length}p = ${extracted}p`);
  return {
...
async function getOrUploadFileUri(fileId, accessToken) {
  ...
  try {
    splitMeta = await splitLargePdf(blob);
...
export async function analyzePdfFromDrive(fileIds, accessToken, vessel = {}) {
  ...
  // 분할 정보 수집 (캐시에서)
  const splitInfo = ids.reduce((acc, id) => {
    const c = _fileUriCache.get(id);
    if (c?.wasSplit) acc = { totalPages: c.totalPages, extractedPages: c.extractedPages, wasSplit: true };
    return acc;
  }, null);
