...
① OPERATION 컬럼: "BALLAST", "DEBALLAST", "STRIPPING", "STANDBY" 등 운전 상태값 포함
...
방법 A (OPERATION 컬럼이 있을 때 — 우선 적용):
  ① OPERATION = "BALLAST" 행의 TRO_B* 컬럼 값 중 0.1~15 사이 값 평균 → ballasting_avg
  ② OPERATION = "DEBALLAST" 행의 TRO_D*(또는 TRO_S*) 컬럼 값 중 0~15 사이 값 평균 → deballasting_avg
  ③ 첫 10분/마지막 10분 행은 제외 (배관 잔류수 구간)
방법 B (OPERATION 컬럼 없거나 방법 A 실패 시):
  ① Step 2의 운전 시작~종료 시간대 Data Log 행 추출
  ② 시작 후 10분 / 종료 전 10분 제외 후 평균
  ③ 안정 구간 데이터 0개 또는 운전 20분 이하 → null
...
Step 3. ECS Data Log 섹션에서 TRO + ECU/FMU 평균 계산
  ...
  - OPERATION 컬럼이 있으면: BALLAST 행 → ballasting_avg, DEBALLAST 행 → deballasting_avg
